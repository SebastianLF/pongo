			...
			if ($count <= 0) {
				...
				if ($validator->fails()) {
					...
					foreach ($selections_coupon as $selection_coupon) {
						...
						if(is_null($scope)){
							$scope = Scope::create(array('id' => $selection_coupon->scope, 'name' => $selection_coupon->name));
						}
						...
						$selection = new Selection(array(
							...
							'live' => $selection_coupon->odd_isLive ? 1 : 0,
							'isMatch' => $selection_coupon->odd_isMatch ? 1 : 0,
